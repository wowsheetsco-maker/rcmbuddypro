...
async function runDispatcher(): Promise<DispatchResult> {
  ...
  for (const [orgId, userIds] of orgMembers.entries()) {
    ...
    // (overdue) or within 2 days (upcoming). The shared helper handles in-app,
    // email (Resend) and WhatsApp (Meta Cloud API) per recipient and audits
    // a `reminder_sent` event with the channels actually used.
    ...
    for (const s of pendingSubs ?? []) {
      try {
        await sendSubmissionReminder(s.id as string, { trigger: "scheduled" });
      } catch (e) {
        console.error("[dispatch-notifications] submission reminder failed", s.id, e);
      }
    }
  }
